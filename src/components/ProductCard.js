...
const ProductCard = ({ product, singleProductStyle }) => {
  return (
    <div
      className={`flex flex-col max-w-sm rounded overflow-hidden shadow-lg border border-gray-300 transition-transform duration-300 transform hover:scale-105 ${singleProductStyle}`}
    >
      <img
        src={product.image}
        alt={product.name}
